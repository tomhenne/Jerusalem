...
public class XMLElement {
	 ...
	 * </dl>
	 */
	private final static Hashtable<String, char[]> defaultEntities = new Hashtable<String, char[]>();
	 ...
	 * <li>getChildren() => empty LinkedList
	 * <li>getContent() => ""
	 * <li>getLineNr() => 0
	 * <li>getName() => null
	 * </ul>
	...
	public XMLElement(boolean skipLeadingWhitespace) {
		this(skipLeadingWhitespace, true);
	}
	 ...
	 *            <li>getChildren() => empty LinkedList <li>getContent() => "" <li>
	 ...
	 */
	protected XMLElement(boolean skipLeadingWhitespace,
			boolean ignoreCase) {
	 ...
	 *      removeChild(XMLElement)
	 */
	public LinkedList<XMLElement> getChildren() {
		try {
			return (LinkedList<XMLElement>) this.children.clone();
		} catch (Exception e) {
	...
	public Map<String, XMLElement> getChildMap() {
		Map<String, XMLElement> params = new HashMap<String, XMLElement>();
		for (XMLElement el : getChildren())
			params.put(el.getName(), el);
	...
	public XMLElement getChild(String name) {
		for (int i = 0; i < children.size(); i++) {
			XMLElement n = (XMLElement) children.get(i);
	...
	public void parseFromReader(Reader reader, int startingLineNr)
		...
		for(;;) {
			...
			if( ch == '<') { this.unreadChar(ch); break; }
		}
	...
	protected XMLElement createAnotherElement() {
		return new XMLElement(this.ignoreWhitespace, 
				this.ignoreCase);
	}
	...
	public void write(Writer writer) throws IOException {
		...
		if ((this.contents != null) && (this.contents.length() > 0)) {
			...
			writer.write('>');
			Iterator<XMLElement> enume = this.getChildrenIterator();
			while (enume.hasNext()) {
				XMLElement child = (XMLElement) enume.next();
				child.write(writer);
			}
			writer.write('<');
			writer.write('/');
	...
	protected void resolveEntity(StringBuffer buf) throws IOException {
		...
		if (key.charAt(0) == '#') {
			...
			char[] value = (char[]) defaultEntities.get(key);
			if (value == null) {
				throw this.unknownEntity(key);
			}
			buf.append(value);
